    ...
    var getHostIP = function(callback) {
      ...
      if (config.settings.lookup) {
        ...
        dns.resolve4(config.account.hosts[0], function(err, addrs) {
          // We assume DynDNS hosts only have one A record
          var response = !err && addrs.length && goodIP(addrs[0]) ? addrs[0] : null;
          updateIP(response);
        });

      } else {
        // Cached IP
        var response = goodIP(config.account.lastKnownIP) ? config.account.lastKnownIP : null;
        updateIP(response);
      }
    };
